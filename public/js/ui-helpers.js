...
function addSecurityWatermark() {
  ...
  watermark.innerHTML = "🛡️ بيئة اختبارات ركائز الآمنة";
  watermark.style.cssText = `position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-45deg);font-size:42px;color:rgba(0,0,0,0.02);pointer-events:none;z-index:-1;font-weight:900;font-family:Cairo;`;
...
function showPage(id) {
  ...
  // SaaS Layout logic: Toggle sidebar only for internal dashboard pages
  const isDashboardPage = ADMIN_PAGES.has(id);
  const sidebar = document.getElementById('admin-sidebar');
  if (sidebar) sidebar.style.display = isDashboardPage ? 'flex' : 'none';
  document.body.classList.toggle('has-sidebar', isDashboardPage);

  // Sync active nav link
  if (isDashboardPage) {
    document.querySelectorAll('.nav-link').forEach(link => {
      const onclick = link.getAttribute('onclick') || '';
      link.classList.toggle('active', onclick.includes(id));
    });
  }
}
...
function buildDashboardCharts(examEntries, allResults) {
  const bySubmissions = [...examEntries].sort((a, b) => b.resultCount - a.resultCount).slice(0, 5).map((exam) => ({ label: exam.title, value: exam.resultCount, meta: `${exam.questionCount} سؤال`, color: "var(--gd)" }));
  const byAverageScore = examEntries.filter((exam) => exam.resultCount > 0).sort((a, b) => b.averageScore - a.averageScore).slice(0, 5).map((exam) => ({ label: exam.title, value: exam.averageScore, meta: `${exam.resultCount} تسليم`, color: "var(--gm)" }));
  const groups = Object.entries(groupResultsByField(allResults, (r) => r.studentGroup)).map(([label, items]) => ({ label, value: items.length, meta: `متوسط ${Math.round(items.reduce((s, i) => s + i.pct, 0) / items.length)}%`, color: "var(--gl)" })).sort((a, b) => b.value - a.value).slice(0, 5);
  return [
    buildBarChartCard("أكثر الاختبارات تسليمًا", bySubmissions, { emptyText: "لا توجد بيانات تسليم حتى الآن.", formatter: (v) => `${v} تسليم` }),
    buildBarChartCard("أعلى متوسطات الأداء", byAverageScore, { emptyText: "سيظهر الرسم البياني بعد رصد أول نتيجة.", formatter: (v) => `${v}%`, color: "var(--gm)" }),
    buildBarChartCard("المجموعات التعليمية النشطة", groups, { emptyText: "رتب طلابك في مجموعات لتظهر الإحصائيات هنا.", formatter: (v) => `${v} طالباً`, color: "var(--gl)" })
  ].join("");
}
